...
核心原則:
- まず、親の文脈でその言葉が具体的に何を指すかを特定する（例: 「最先端モデル」→この文脈ではFable 5とMythos 5）
- その上で、親が触れなかった仕組み・背景・具体例・数値・比較を足す
- 親の要約や言い換えは深化ではない

応答ルール:
- 300〜500字。前置きや挨拶は書かない。
- 親の文脈での具体的な指示対象を最初に明確にし、そこから掘り下げる。辞書的な一般論で終わらない。
...
function buildUserPrompt(req: ExpandRequest): string {
  ...
  if (req.operation === "root") {
    ...
    parts.push(
      req.operation === "what"
        ? `[操作] What is it — 親本文の文脈で「${req.selectedSpan}」が具体的に何を指すかを特定し（固有名詞・製品名・事例など）、その上でそれが何であるかを掘り下げる。一般的な定義ではなく、この文脈での具体的な実体から始める。`
        : `[操作] Why is it — 親本文の文脈で、なぜ「${req.selectedSpan}」なのかを掘り下げる。根拠・原理・歴史的背景・代替との比較を、この文脈の具体例に即して説明する。`
    );
  }
  return parts.join("\n\n");
}
