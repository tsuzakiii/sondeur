...
Free plan exposure (dimension varies — signed-in Free is per-user, guest is per-IP):

- **Free (signed-in) user, monthly**: capped at 20 nodes/month by the `consume_node_quota` RPC (`supabase/migrations/0002_node_quota.sql`) enforced server-side against `PLAN_NODE_LIMITS.free = 20` in `src/lib/planLimits.ts`. Worst case at 20 all-search nodes: about $0.45. The counter resets on each new `month_key`. The `consumeNodeQuota` helper is fail-open: if the profile row cannot be read or the RPC itself errors, the request is allowed to proceed and the error is captured via `Sentry.captureException` (`src/lib/planLimits.ts`). Runbook item: monitor Sentry for this class of error before treating the cap as a hard bound.
- **Guest, per-IP per-day**: the server-side hard limit is `DAILY_LIMIT = 15` in `src/lib/guestRateLimit.ts`, enforced by the `consume_guest_quota(p_ip_hash, p_limit)` RPC from `supabase/migrations/0005_guest_rate_limit.sql`. The counter is keyed by SHA-256 of the client IP and a `day date` column, so it **resets every calendar day**. Worst case at 15 all-search nodes per day: about $0.34/day per IP, recurring — not a one-shot ceiling. `checkGuestRateLimit` is fail-open in two distinct ways: (a) when `SUPABASE_SECRET_KEY` is not configured the function returns `true` silently — this is intentional so dev environments without Supabase still work, and production is guarded upstream by `release_check.mjs` which fails the launch check if that env var is missing; (b) when the RPC errors at runtime the error is logged via `console.error` and the request is allowed (no Sentry capture on this path — a separate follow-up if we want to alert on it). Do not treat the 15/day figure as absolute in production until both paths are audited.
- `GUEST_NODE_LIMIT = 10` in `src/lib/planLimits.ts` is a **client-side friction check** consumed by `src/app/page.tsx`. It reads `totalNodes` from localStorage and gates the UI, but a determined user can bypass it by clearing localStorage or switching browser profiles. The 15-per-day server-side RPC is the actual bound (subject to the fail-open caveats above).
