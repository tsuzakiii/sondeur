...
3. **AC-#15-3** — `src/lib/inFlightSession.ts` (new) exports:
   ...
   - `expireInFlightSessionIfDifferentPlan(stripe, sessionId, requestedPrice): Promise<"cleared" | "kept-same-plan">` — retrieves the Session, compares prices, calls `stripe.checkout.sessions.expire` when different. On expire throwing `code: "session_already_expired"` returns `"cleared"`. On `code: "session_already_completed"` throws `new SessionAlreadyCompletedError()`. On any other error rethrows.
...
- **impl-r7-F2** — `expireInFlightSessionIfDifferentPlan` on same-plan match must also inspect `retrieved.status`. If the Session is `"expired"` or `"complete"`, clear the DB pointer via `clearInFlightSession(userId, sessionId)` and return `"cleared"` (not `"kept-same-plan"`); do NOT preserve a stale pointer that a subsequent same-plan click would try to reuse via idempotency (Stripe would replay the expired Session).
