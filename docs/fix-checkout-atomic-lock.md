...
2. **AC-#15-2** — `src/lib/checkoutLock.ts` (new) exports:
   - `tryAcquireCheckoutLock(userId): Promise<string | null | "unavailable">` — calls `try_acquire_checkout_lock` RPC via `getServiceSupabase()`. Returns `"unavailable"` (fail-CLOSED) both when the service client is null AND when the RPC returns an error / throws (with `Sentry.captureException`) — the "unavailable" signal is what the route maps to 503, so infra failures / missing migration are surfaced to the operator rather than masked as 409 contention. Returns `null` only on a genuine race loser (RPC returned no row). Returns the token string on success.
   ...
   Unit tests in `src/lib/__tests__/checkoutLock.test.ts` (7 cases): (a) service client returns token → acquire returns token; (b) service client returns `{ data: null, error: null }` → acquire returns `null` (race loser); (c) `getServiceSupabase()` returns null → acquire returns `"unavailable"`, no RPC call attempted; (d) service client returns error → acquire returns `"unavailable"` and `Sentry.captureException` called; (d') RPC throws → acquire returns `"unavailable"` and `Sentry.captureException` called; (e) release with matching token → RPC called, promise resolves; (f) release with service client error → promise still resolves, `Sentry.captureException` called.
...
3. **AC-#15-3** — `src/lib/inFlightSession.ts` (new) exports:
   ...
   - `expireInFlightSessionIfDifferentPlan(stripe, supabase, userId, sessionId, requestedPrice): Promise<"cleared" | "kept-same-plan">` — retrieves the Session (with `expand: ["line_items"]`), compares prices, calls `stripe.checkout.sessions.expire` when different. Clears the DB pointer on any `"cleared"` path via a private `safeClearInFlightSession` wrapper — that wrapper catches Supabase errors and forwards them to `Sentry.captureException` without rethrowing. This is a deliberate asymmetry: the Stripe API side (`expire`) has already succeeded (or returned `already_expired`), so the invariant "Session is no longer payable" is already met; failing the whole route because the DB pointer clear had a transient blip would strand the caller with a 500 on a request that actually succeeded on the payments side. The uncleared pointer self-heals on the next checkout's cleanup (retrieve → status expired → clear again). On expire throwing `code: "session_already_expired"` returns `"cleared"`. On `code: "session_already_completed"` throws `new SessionAlreadyCompletedError()`. On any other error rethrows. If the retrieved Session's `status` is `"expired"` clears and returns `"cleared"`; if `"complete"` throws `SessionAlreadyCompletedError`.
   - The webhook side uses the throwing `clearInFlightSession` directly and translates any error to `ok = false` so Stripe retries the delivery — the semantics there are different because the webhook's job is precisely to keep the DB in sync, and a silent 200 with a stale pointer produces the permanent 409 stuck path that B1 of the r1 review flagged.
   ...
   Unit tests in `src/lib/__tests__/inFlightSession.test.ts` cover: (a) different plan → expire is called, returns `"cleared"`; (b) same plan → expire NOT called, returns `"kept-same-plan"`; (c) expire throws `code: "session_already_expired"` → returns `"cleared"`; (d) expire throws `code: "session_already_completed"` → throws `SessionAlreadyCompletedError` (assert `instanceof`); (e) expire throws anything else → the helper rethrows the original error; (f) `recordInFlightSession` UPDATE calls the correct SQL shape and throws on Supabase error; (g) `clearInFlightSession` throws on Supabase error (no silent swallow), UPDATE shape includes both `.eq("id", ...)` and `.eq("in_flight_checkout_session_id", ...)` filters — the WHERE-guard's runtime behavior (mismatched Session ID = 0 rows updated) is guaranteed by Postgres; the unit test asserts the client-side wiring that gates it; (h) `safeClearInFlightSession` (used by the route-side paths at `inFlightSession.ts:65` and `:87`) catches a Supabase error from the underlying clear and still returns `"cleared"` — this is the regression net for the intentional route/webhook asymmetry, so a future edit reverting either call site back to the throwing `clearInFlightSession` fails the test suite.
...
5. **AC-#15-5** — `src/app/api/billing/webhook/route.ts` clears `in_flight_checkout_session_id` via `clearInFlightSession(supabase, userId, event.data.object.id)` on:
   ...
   Both webhook call sites wrap `clearInFlightSession` in `try/catch`. On a caught Supabase error the webhook `console.error`s, calls `Sentry.captureException(clearErr)`, and sets `ok = false`. `ok = false` causes the handler to return 500, so Stripe retries the webhook delivery — this is the load-bearing behavior that prevents the "permanent 409 stuck" path r1 of pr-merge-review flagged for PR #18: a silent 200 with a stale pointer would leave the affected user unable to check out, and a `"complete"` status pointer has no self-heal path (unlike `"expired"`). The asymmetry with the route side (which uses `safeClearInFlightSession` and swallows) is intentional and required.
